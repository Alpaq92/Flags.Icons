...
namespace Flags.Icons {
    /// <summary>
    /// Helpers consumed by the per-stack <c>FlagIcon</c> controls, which all expose 4 typed
    /// source properties (Twemoji / Circle / Square / Lipis) and need to dispatch on whichever
    /// one is currently non-<c>None</c>. Extracted to keep the 8 control implementations DRY.
    /// </summary>
    public static class FlagSourceDispatch {
        /// <summary>
        /// Opens the embedded SVG stream for whichever of the 4 arguments is non-<c>None</c>, in
        /// priority order (Twemoji → Circle → Square → Lipis). Returns <c>null</c> if all are <c>None</c>.
        /// </summary>
        public static Stream? OpenActive(TwemojiFlag twemoji, CircleFlag circle, SquareFlag square, LipisFlag lipis) {
        ...
        /// 4 are <c>None</c>.
        /// </summary>
        public static (FlagSource source, int value)? GetActive(TwemojiFlag twemoji, CircleFlag circle, SquareFlag square, LipisFlag lipis) {
